    ...
    def visualize_coarsening(G, G_coarse, c_data):
        ...
        for seed in seeds:
            ...
            for i, val in enumerate(rnd_color):
                color_seed[:, i] *= val
                color_rest[:, i] *= val
            nx.draw_networkx_nodes(G, pos=pos, nodelist=[seed], node_color=color_seed, cmap=pylab.hot, node_size=500,
                                   with_labels=True, node_shape='s')
            nx.draw_networkx_nodes(G, pos=pos, nodelist=trapped_nodes, node_color=color_rest, cmap=pylab.hot,
                                   node_size=200, with_labels=True, node_shape='o')
...
def edge_attachment_test(editing_demo_draw_func, seed=None):
    ...
    print
    'All new edges should lie close to the cycle'
...
def evaluate_similarity(base_graphs, graphs, sim_metrics=None, n_jobs=-1):
    ...
    if n_jobs == 1:  # other values are meaningful for joblib
        ...
        print
        'Running parallel SIMILARITY MEASUREMENT ...'
        sys.stdout.flush()
        graph_data = Parallel(n_jobs=n_jobs, verbose=True)(
            delayed(safe_similarity)(base_graphs[graph_i], graph, sim_metrics) for graph_i, graph in enumerate(graphs))
        for rets in graph_data:
            for met_num, metric in enumerate(sim_metrics):
                vals_of_graphs[met_num].append(rets[met_num])
        sys.stdout.flush()
...
def param_set_generator(default_params=None, base_vectors=None, edit_amplitude=0.05, fixed_set=None):
    ...
    if default_params == None:
        default_params = {'num_v_cycles': 10, 'verbose': False, 'dont_cutoff_leafs': False, 'enforce_connected': True,
                          'accept_chance_edges': 1.0, }

    all_option_sets = [p for p in itertools.product(*tuple(v['value_options'] for v in base_vectors))]
    print
    'Total # of parameter sets: %d' % len(all_option_sets)
...
def plot_deviation(vals_of_replicas, vals_of_graph, metrics, figpath, jaccard_edges=None, title_infix='', seed=0,
                   Gname=''):
    # vals_of_graph could be a number (level 0) or a list (the same as the number of replicas)
    clean_names = {'num nodes': 'num nodes', 'num edges': 'num edges', 'clustering': 'clustering',
                   'average degree': 'avg\ndegree', 'degree assortativity': 'degree\nassortativity',
                   'degree connectivity': 'degree\nconnectivity',
                   'total deg*deg': 'total deg*deg\nassortativity',
                   's-metric': 's metric', 'mean ecc': 'avg\neccentricity', 'num comps': 'num comps',
    ...
    replica_stats = {'median_of_replicas': med_vals, 'avg_of_replicas': avg_vals, 'p25_of_replicas': p25_vals,
                     'p75_of_replicas': p75_vals, 'max_of_replicas': max_vals, 'min_of_replicas': min_vals,
                     'std_of_replicas': std_vals}

    normed_replica_vals = []
    avg_norms = []
    print
    'Medians' + (' (average of model graphs)' if multiple_models else '')
    print
    '-------'
    print
    'metric\t\tOriginalG\t\tReplicas'
    for met_num, metric in enumerate(metrics):
        try:
            model_val = np.average(vals_of_graph[met_num]) if multiple_models else vals_of_graph[met_num]
            print
            '%s\t\t%.5f\t\t%.5f' % (metric['name'], model_val, med_vals[met_num])
        except:
            print
            '%\tserror' % metric['name']
    for met_num, metric in enumerate(metrics):
...
def replicate_graph(G, generator_func, num_replicas, params, title_infix='', n_jobs=-1):
    if n_jobs == 1:  # other values are meaningful for joblib
        ...
        replicas = Parallel(n_jobs=n_jobs, verbose=True)(
            delayed(generator_func)(G, params) for i in xrange(num_replicas))
        print
        '   %d replicas done' % len(replicas)
        sys.stdout.flush()
...
def replica_vs_original(seed=None, figpath=None, generator_func=None, G=None, params=None, num_replicas=150,
    ...
    if params == None:
        params = {'verbose': False, 'node_edit_rate': [0.05, 0.04, 0.03, 0.02, 0.01],
                  'edge_edit_rate': [0.05, 0.04, 0.03, 0.02, 0.01], 'node_growth_rate': [0],
                  'locality_bias_correction': 0., 'enforce_connected': True, 'accept_chance_edges': 1.0,
...
def save_figure_helper(fpath):
    if matplotlib.get_backend() == 'pdf':
        ...
        print
        'Trying to save to PDF.  Backend: %s' % matplotlib.get_backend()
        final_path = fpath + '.pdf'
        pylab.savefig(final_path)
        pylab.close()
        print
...
def save_param_set(param_set, seed, fpath):
    with open(fpath + '_params.txt', 'w') as f:
        ...
        print
        'Written parameter set to: %s' % f.name
...
def save_stats_csv(path, seed, data):
    ...
    rets.append(timeNow())
    header.append('graph name');
    rets.append(data['name'])
    header.append('figpath');
    rets.append(data['figpath'])
    header.append('seed');
    rets.append(seed)
    ...
    for level in xrange(100):
        ...
        header.append('level%d' % level);
        rets.append('--->')
        header.append('avg_jaccard_edges');
        rets.append(level_stats['avg_jaccard_edges'])
        header.append('mean_mean_error');
        rets.append(level_stats['mean_mean_error'])
        header.append('mean_mean_errorstd');
        rets.append(level_stats['mean_mean_errorstd'])
        num_metrics = len(metric_names)
        distributional_stats = [k for k in level_stats if
                                hasattr(level_stats[k], '__len__')]  # e.g. median, avg, min, max ...
...
def statistical_tests(seed=8):
    ...
    params_default = {'verbose': False, 'edge_edit_rate': [0.08, 0.07], 'node_edit_rate': [0.08, 0.07],
                      'node_growth_rate': [0],
                      'dont_cutoff_leafs': False,
                      'new_edge_horizon': 10, 'num_deletion_trials': 20, 'locality_bias_correction': [0, ],
                      'edit_method': 'sequential',
                      }
    ...
    metrics_default = filter(
        lambda met: met['name'] not in ['avg flow closeness', 'avg eigvec centrality', 'degree connectivity',
                                        'degree assortativity', 'average shortest path', 'mean ecc', 'powerlaw exp', ],
        metrics_default)
    ...
    for problem in problems:
        ...
        replica_vs_original(G=base_graph, num_replicas=num_replicas, seed=1, params=params, metrics=metrics,
